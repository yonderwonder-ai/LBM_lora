...
class LoRALinear(nn.Module):
    ...
    def forward(self, x):
        scaling = self.alpha / self.lora_down.out_features
        return self.linear(x) + (self.lora_up(self.lora_down(x)) * scaling)
...
class LoRAConv2d(nn.Module):
    ...
    def forward(self, x):
        scaling = self.alpha / self.lora_down.out_channels
        return self.conv(x) + (self.lora_up(self.lora_down(x)) * scaling)
